...
def _load_adapter():
    module_name = "plugin_adapter_qqbot_plus_default_discovery_test"
    cached = sys.modules.get(module_name)
    if cached is not None:
        return cached
    adapter_path = Path(__file__).parents[2] / "plugins" / "platforms" / "qqbot-plus" / "adapter.py"
...
async def test_send_clarify_with_choices_uses_keyboard_not_streaming():
    ...
    assert result.success is True
    adapter.send_with_keyboard.assert_awaited_once()
    args = adapter.send_with_keyboard.await_args.args
    assert args[0] == "user-1"
    assert "Pick one" in args[1]
    keyboard = args[2]
    datas = [row.buttons[0].action.data for row in keyboard.content.rows]
    assert datas == ["clarify:cid-1:0", "clarify:cid-1:1", "clarify:cid-1:other"]
...
async def test_send_clarify_open_ended_uses_non_streaming_text_send():
    ...
    assert qq_send.await_args.args[1:3] == ("user-1", "❓ What should I do?")
    assert qq_send.await_args.kwargs["metadata"] == {
        "source": "clarify",
        "streaming": False,
    }
...
async def test_send_clarify_keyboard_failure_marks_text_fallback():
    ...
    with patch("tools.clarify_gateway.mark_awaiting_text", return_value=True) as mark_text, \
         patch.object(_qqbot_plus.QQAdapter, "send", new=AsyncMock(return_value=SimpleNamespace(success=True, message_id="fallback-1"))) as qq_send:
        result = await adapter.send_clarify(
            chat_id="user-1",
            question="Pick one",
            choices=["Alpha", "Beta"],
    ...
    assert result.success is True
    mark_text.assert_called_once_with("cid-3")
    qq_send.assert_awaited_once()
    assert "1. Alpha" in qq_send.await_args.args[2]
    assert qq_send.await_args.kwargs["metadata"] == {"streaming": False}
...
@pytest.mark.parametrize(
    ("method_name", "path_kwarg", "path_value", "kind"),
    [
        ("send_image_file", "image_path", "/tmp/example.png", "image"),
        ("send_voice", "audio_path", "/tmp/example.ogg", "voice"),
        ("send_video", "video_path", "/tmp/example.mp4", "video"),
        ("send_document", "file_path", "/tmp/example.pdf", "file"),
    ],
)
async def test_media_methods_reuse_qq_rich_media_upload_with_tool_metadata(
    ...
    assert args[0] == "user-1"
    assert args[1] == path_value
    assert args[3] == kind


@pytest.mark.asyncio
async def test_send_voice_uses_qq_voice_media_type():
    ...
    send_media.assert_awaited_once()
    args = send_media.await_args.args
    assert args[2] == MEDIA_TYPE_VOICE
    assert args[3] == "voice"
